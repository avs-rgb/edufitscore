...
function nextSeason(season) {
  ...
  return Number.isInteger(startYear) ? `${startYear + 1}-${startYear + 2}` : seasonForDate();
}

function previousSeason(season) {
  const startYear = Number(String(season || '').split('-')[0]);
  return Number.isInteger(startYear) ? `${startYear - 1}-${startYear}` : initialSeason;
}

function normalizeSeason(value) {
  const text = String(value || '').trim();
  return /^\d{4}-\d{4}$/.test(text) ? text : seasonForDate();
}

function isSeasonLocked(season, date = new Date()) {
  return normalizeSeason(season) !== seasonForDate(date);
}

function signupStartSeason(date = new Date()) {
  const current = seasonForDate(date);
  return date.getUTCMonth() === 6 ? nextSeason(current) : current;
}
...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    billing_exempt_at TEXT,
    billing_exempt_by_user_id INTEGER,
    paid_until TEXT,
...
if (!teacherClassesInfo.some((column) => column.name === 'season')) {
  db.exec("ALTER TABLE teacher_classes ADD COLUMN season TEXT NOT NULL DEFAULT '2025-2026'");
}
db.exec("UPDATE teacher_classes SET season = '2025-2026' WHERE season IS NULL OR season = ''");
db.exec('CREATE INDEX IF NOT EXISTS teacher_classes_user_season_idx ON teacher_classes(user_id, season, order_index)');
...
if (!usersInfo.some((column) => column.name === 'trial_started_at')) {
  db.exec('ALTER TABLE users ADD COLUMN trial_started_at TEXT');
}
if (!usersInfo.some((column) => column.name === 'trial_ends_at')) {
  db.exec('ALTER TABLE users ADD COLUMN trial_ends_at TEXT');
}
if (!usersInfo.some((column) => column.name === 'billing_status')) {
  db.exec("ALTER TABLE users ADD COLUMN billing_status TEXT NOT NULL DEFAULT 'exempt'");
}
if (!usersInfo.some((column) => column.name === 'billing_exempt')) {
  db.exec('ALTER TABLE users ADD COLUMN billing_exempt INTEGER NOT NULL DEFAULT 1');
}
if (!usersInfo.some((column) => column.name === 'billing_exempt_at')) {
  db.exec('ALTER TABLE users ADD COLUMN billing_exempt_at TEXT');
}
if (!usersInfo.some((column) => column.name === 'billing_exempt_by_user_id')) {
  db.exec('ALTER TABLE users ADD COLUMN billing_exempt_by_user_id INTEGER');
}
if (!usersInfo.some((column) => column.name === 'paid_until')) {
  db.exec('ALTER TABLE users ADD COLUMN paid_until TEXT');
}
if (!usersInfo.some((column) => column.name === 'payment_customer_id')) {
  db.exec("ALTER TABLE users ADD COLUMN payment_customer_id TEXT NOT NULL DEFAULT ''");
}
if (!usersInfo.some((column) => column.name === 'payment_session_id')) {
  db.exec("ALTER TABLE users ADD COLUMN payment_session_id TEXT NOT NULL DEFAULT ''");
}
if (!usersInfo.some((column) => column.name === 'billing_updated_at')) {
  db.exec('ALTER TABLE users ADD COLUMN billing_updated_at TEXT');
}
db.exec("UPDATE users SET email_verified = 1, email_verified_at = COALESCE(email_verified_at, updated_at) WHERE role = 'admin'");
db.prepare("UPDATE users SET billing_exempt = 1, billing_status = 'exempt', billing_exempt_at = COALESCE(billing_exempt_at, created_at), billing_updated_at = COALESCE(billing_updated_at, updated_at) WHERE role = 'admin'").run();
db.prepare("UPDATE users SET billing_exempt = 0, billing_status = 'active', billing_exempt_at = NULL, billing_exempt_by_user_id = NULL, paid_until = COALESCE(paid_until, ?), billing_updated_at = ? WHERE role != 'admin' AND billing_exempt = 1").run(billingAcademicYearEndForDate(), nowIso());
db.prepare("UPDATE users SET billing_exempt = 0, billing_status = 'active', paid_until = COALESCE(paid_until, ?), billing_updated_at = COALESCE(billing_updated_at, updated_at) WHERE role != 'admin' AND billing_updated_at IS NULL").run(billingAcademicYearEndForDate());
db.prepare(`
  UPDATE users
  SET first_name = CASE
...
db.prepare("DELETE FROM users WHERE email IN ('teacher@edufitscore.local')").run();
seedGlobalAdmin();
...
function serializeUser(row) {
  ...
  const approvedTeacherSchool = db.prepare(`
    ...
    LIMIT 1
  `).get(row.id);
  const billing = billingStateForUser(row);
  ...
  return {
    ...
    canEnterScores: row.role === 'admin' || row.role === 'teacher' || Boolean(schoolAdmin) || Boolean(approvedTeacherSchool),
    billing,
...
function createUser(payload) {
  ...
  const timestamp = nowIso();
  const trialEndsAt = addDaysIso(timestamp, 30);
  const passwordHash = bcrypt.hashSync(payload.password, 10);
  const result = db.prepare(`
    INSERT INTO users (email, password_hash, role, full_name, first_name, last_name, phone, city, school_name, email_verified, email_verified_at, is_active, trial_started_at, trial_ends_at, billing_status, billing_exempt, billing_updated_at, created_at, updated_at)
    VALUES (?, ?, 'teacher', ?, ?, ?, ?, ?, ?, 0, NULL, 1, ?, ?, 'trialing', 0, ?, ?, ?)
  `).run(
    email,
    passwordHash,
...
function setUserBillingExempt(userId, exempt, adminUserId = null) {
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(userId);
  if (!row) throw new Error('USER_NOT_FOUND');
  if (row.role !== 'admin') throw new Error('BILLING_EXEMPT_DISABLED');
  if (row.role === 'admin' && !exempt) throw new Error('ADMIN_BILLING_EXEMPT_REQUIRED');
  const timestamp = nowIso();
  db.prepare(`
    UPDATE users
    SET billing_exempt = ?, billing_status = ?, billing_exempt_at = ?, billing_exempt_by_user_id = ?, billing_updated_at = ?, updated_at = ?
    WHERE id = ?
  `).run(exempt ? 1 : 0, exempt ? 'exempt' : 'trialing', exempt ? timestamp : null, exempt ? adminUserId : null, timestamp, timestamp, userId);
  return getUserById(userId);
}

function extendUserTrial(userId, days = 30) {
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(userId);
  if (!row) throw new Error('USER_NOT_FOUND');
  const timestamp = nowIso();
  const currentEnd = row.trial_ends_at && Date.parse(row.trial_ends_at) > Date.now() ? row.trial_ends_at : timestamp;
  const trialEndsAt = minIso(addDaysIso(currentEnd, days), billingAcademicYearEndForDate(timestamp));
  db.prepare(`
    UPDATE users
    SET trial_started_at = COALESCE(trial_started_at, ?), trial_ends_at = ?, billing_status = 'trialing', billing_updated_at = ?, updated_at = ?
    WHERE id = ?
  `).run(timestamp, trialEndsAt, timestamp, timestamp, userId);
  return getUserById(userId);
}

function extendUserToAcademicYearEnd(userId, value = new Date()) {
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(userId);
  if (!row) throw new Error('USER_NOT_FOUND');
  const timestamp = nowIso();
  const paidUntil = billingAcademicYearEndForDate(value);
  db.prepare(`
    UPDATE users
    SET paid_until = ?, billing_status = 'active', billing_updated_at = ?, updated_at = ?
    WHERE id = ?
  `).run(paidUntil, timestamp, timestamp, userId);
  return getUserById(userId);
}

function cancelUserAcademicYearAccess(userId) {
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(userId);
  if (!row) throw new Error('USER_NOT_FOUND');
  if (row.role === 'admin') throw new Error('ADMIN_BILLING_EXEMPT_REQUIRED');
  const timestamp = nowIso();
  db.prepare(`
    UPDATE users
    SET paid_until = NULL, trial_started_at = NULL, trial_ends_at = NULL, billing_status = 'expired', billing_exempt = 0, billing_updated_at = ?, updated_at = ?
    WHERE id = ?
  `).run(timestamp, timestamp, userId);
  return getUserById(userId);
}

function savePaymentSession(userId, { customerId = '', sessionId = '' } = {}) {
  const timestamp = nowIso();
  db.prepare(`
    UPDATE users
    SET payment_customer_id = COALESCE(NULLIF(?, ''), payment_customer_id), payment_session_id = ?, billing_updated_at = ?, updated_at = ?
    WHERE id = ?
  `).run(String(customerId || ''), String(sessionId || ''), timestamp, timestamp, userId);
  return getUserById(userId);
}

function markUserPaidForAcademicYear(userId, { customerId = '', sessionId = '', paidAt = new Date() } = {}) {
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(userId);
  if (!row) throw new Error('USER_NOT_FOUND');
  const timestamp = nowIso();
  const paidUntil = billingAcademicYearEndForDate(paidAt);
  db.prepare(`
    UPDATE users
    SET paid_until = ?, billing_status = 'active', billing_exempt = 0, payment_customer_id = COALESCE(NULLIF(?, ''), payment_customer_id), payment_session_id = COALESCE(NULLIF(?, ''), payment_session_id), billing_updated_at = ?, updated_at = ?
    WHERE id = ?
  `).run(paidUntil, String(customerId || ''), String(sessionId || ''), timestamp, timestamp, userId);
  return getUserById(userId);
}
...
function replaceAdminTwoFactorRecoveryCodes(userId, recoveryCodes) {
...
function deleteExpiredTwoFactorRecoveryRegenerationTokens() {
  db.prepare('DELETE FROM admin_2fa_recovery_regeneration_tokens WHERE expires_at <= ? OR used_at IS NOT NULL').run(nowIso());
}

function createTwoFactorRecoveryRegenerationToken(userId, details = {}) {
  deleteExpiredTwoFactorRecoveryRegenerationTokens();
  const row = db.prepare("SELECT id, email FROM users WHERE id = ? AND role = 'admin' AND is_active = 1 AND totp_enabled = 1").get(userId);
  if (!row) {
    throw new Error('TWO_FACTOR_NOT_ENABLED');
  }
  const token = crypto.randomBytes(32).toString('base64url');
  ...
  db.prepare('DELETE FROM admin_2fa_recovery_regeneration_tokens WHERE user_id = ?').run(userId);
  db.prepare(`
    INSERT INTO admin_2fa_recovery_regeneration_tokens (token_hash, user_id, expires_at, created_at, ip_address, user_agent)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(tokenHash, userId, expiresAt, createdAt, String(details.ip || '').slice(0, 120), String(details.userAgent || '').slice(0, 500));
  return { token, expiresAt, user: { id: row.id, email: row.email } };
}

function getTwoFactorRecoveryRegenerationToken(token) {
  deleteExpiredTwoFactorRecoveryRegenerationTokens();
  if (!token) {
    return null;
  }
  const row = db.prepare(`
    ...
    WHERE admin_2fa_recovery_regeneration_tokens.token_hash = ?
      AND admin_2fa_recovery_regeneration_tokens.used_at IS NULL
      AND admin_2fa_recovery_regeneration_tokens.expires_at > ?
      AND users.is_active = 1
      AND users.role = 'admin'
      AND users.totp_enabled = 1
  `).get(hashTwoFactorToken(token), nowIso());
...
function consumeTwoFactorRecoveryRegenerationToken(token) {
  if (!token) return;
  db.prepare('UPDATE admin_2fa_recovery_regeneration_tokens SET used_at = ? WHERE token_hash = ?').run(nowIso(), hashTwoFactorToken(token));
}
...
function hasRecentPasswordResetToken(email, sinceIso) {
  const row = findUserByEmail(email);
  if (!row) return false;
  return Boolean(db.prepare(`
    SELECT 1
    ...
    LIMIT 1
  `).get(row.id, sinceIso || new Date(0).toISOString()));
}
...
function logAdminAction(adminUserId, targetUserId, action, details = {}) {
  const retentionDays = Number(process.env.ADMIN_AUDIT_RETENTION_DAYS || 365);
  if (retentionDays > 0) {
    db.prepare('DELETE FROM admin_audit_log WHERE created_at < ?').run(new Date(Date.now() - (retentionDays * 24 * 60 * 60 * 1000)).toISOString());
  }
  db.prepare(`
...
function listAdminAuditLogFiltered(options = {}) {
  ...
  if (actions.length) {
    where.push(`admin_audit_log.action IN (${actions.map(() => '?').join(', ')})`);
    params.push(...actions);
  }
  if (from) {
    where.push('admin_audit_log.created_at >= ?');
    params.push(from);
  }
  if (to) {
    where.push('admin_audit_log.created_at <= ?');
  ...
  if (search) {
    where.push('(LOWER(admin_audit_log.action) LIKE ? OR LOWER(admin.email) LIKE ? OR LOWER(admin.full_name) LIKE ? OR LOWER(target.email) LIKE ? OR LOWER(target.full_name) LIKE ? OR LOWER(admin_audit_log.details_json) LIKE ?)');
...
function countRecentAdminAuditActions(action, detailsMatch = {}, sinceIso = '') {
  const rows = db.prepare('SELECT details_json FROM admin_audit_log WHERE action = ? AND created_at >= ?').all(action, sinceIso || new Date(0).toISOString());
  return rows.filter((row) => {
    const details = JSON.parse(row.details_json || '{}');
    return Object.entries(detailsMatch || {}).every(([key, value]) => String(details[key] || '') === String(value || ''));
  }).length;
}

function incrementRateLimit(key, windowMs) {
  const cleanKey = String(key || '').slice(0, 240);
  const now = nowIso();
  const resetAt = new Date(Date.now() + Number(windowMs || 60000)).toISOString();
  db.prepare('DELETE FROM rate_limits WHERE reset_at <= ?').run(now);

  const existing = db.prepare('SELECT count, reset_at FROM rate_limits WHERE key = ?').get(cleanKey);
  if (!existing) {
    db.prepare('INSERT INTO rate_limits (key, count, reset_at, updated_at) VALUES (?, 1, ?, ?)').run(cleanKey, resetAt, now);
    return { count: 1, resetAt };
  }

  const count = Number(existing.count || 0) + 1;
  db.prepare('UPDATE rate_limits SET count = ?, updated_at = ? WHERE key = ?').run(count, now, cleanKey);
...
function exportBackupData() {
  return {
    exportedAt: nowIso(),
    users: db.prepare('SELECT id, email, password_hash, role, full_name, first_name, last_name, phone, city, school_name, must_change_password, totp_secret, totp_enabled, totp_enabled_at, email_verified, email_verified_at, is_active, trial_started_at, trial_ends_at, billing_status, billing_exempt, billing_exempt_at, billing_exempt_by_user_id, paid_until, payment_customer_id, payment_session_id, billing_updated_at, created_at, updated_at FROM users ORDER BY id').all(),
...
function summarizeBackupData(backup) {
  ...
  return {
    ...
    schoolMemberships: (backup.schoolMemberships || []).length,
    schoolScoreTables: (backup.schoolScoreTables || []).length,
    ...
    auditLogEntries: (backup.adminAuditLog || []).length,
    emailVerificationTokens: (backup.emailVerificationTokens || []).length,
    passwordResetTokens: (backup.passwordResetTokens || []).length,
    twoFactorRecoveryCodes: (backup.adminTwoFactorRecoveryCodes || []).length,
  };
}

function restoreBackupData(backup, adminUserId = null) {
  summarizeBackupData(backup);
  ...
  const restore = db.transaction(() => {
    ...
    const insertUser = db.prepare(`
      INSERT INTO users (id, email, password_hash, role, full_name, first_name, last_name, phone, city, school_name, must_change_password, totp_secret, totp_enabled, totp_enabled_at, email_verified, email_verified_at, is_active, trial_started_at, trial_ends_at, billing_status, billing_exempt, billing_exempt_at, billing_exempt_by_user_id, paid_until, payment_customer_id, payment_session_id, billing_updated_at, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    backup.users.forEach((user) => {
      ...
      insertUser.run(
        ...
        user.trial_started_at || user.trialStartedAt || null,
        user.trial_ends_at || user.trialEndsAt || null,
        user.billing_status || user.billingStatus || (user.role === 'admin' ? 'exempt' : 'exempt'),
        user.billing_exempt === undefined ? (user.billingExempt === false ? 0 : 1) : (user.billing_exempt ? 1 : 0),
        user.billing_exempt_at || user.billingExemptAt || (user.billing_exempt || user.billingExempt ? (user.created_at || user.createdAt || nowIso()) : null),
        user.billing_exempt_by_user_id || user.billingExemptByUserId || null,
        user.paid_until || user.paidUntil || null,
        user.payment_customer_id || user.paymentCustomerId || user.stripe_customer_id || user.stripeCustomerId || '',
        user.payment_session_id || user.paymentSessionId || user.stripe_checkout_session_id || user.stripeCheckoutSessionId || '',
        user.billing_updated_at || user.billingUpdatedAt || user.updated_at || user.updatedAt || nowIso(),
    ...
    const insertClass = db.prepare(`
      INSERT INTO teacher_classes (id, user_id, name, grade, gender, season, school_id, order_index, student_count, roster_json, values_json, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    backup.teacherClasses.forEach((teacherClass) => {
      insertClass.run(
        ...
        teacherClass.grade,
        teacherClass.gender,
        teacherClass.season || initialSeason,
...
function createSession(userId, details = {}) {
  ...
  const createdAt = nowIso();
  const expiresInDays = Math.max(Number(details.expiresInDays || 7), 1);
...
function findUserBySessionToken(token) {
  ...
  const row = db.prepare(`
    SELECT users.*, sessions.last_seen_at AS session_last_seen_at, sessions.created_at AS session_created_at, sessions.expires_at AS session_expires_at
    FROM sessions
    JOIN users ON users.id = sessions.user_id
    WHERE sessions.token = ?
      AND sessions.expires_at > ?
...
function deleteUserSessionByPublicId(userId, sessionId, currentToken = '') {
  const row = db.prepare('SELECT token FROM sessions WHERE user_id = ?').all(userId).find((session) => publicSessionId(session.token) === sessionId);
  if (!row || row.token === currentToken) {
    return false;
  }
  const result = db.prepare('DELETE FROM sessions WHERE user_id = ? AND token = ?').run(userId, row.token);
  return result.changes > 0;
}
...
function serializeTeacherClass(row) {
  ...
  return {
    ...
    grade: row.grade,
    gender: row.gender,
    season: row.season || initialSeason,
...
function listTeacherClassesForSeason(userId, season) {
  const rows = db.prepare(`
    SELECT *
  ...
  return rows.map(serializeTeacherClass);
}

function listTeacherSeasons(userId) {
  const seasons = db.prepare('SELECT DISTINCT season FROM teacher_classes WHERE user_id = ? ORDER BY season DESC').all(userId).map((row) => row.season || initialSeason);
  const current = seasonForDate();
  return Array.from(new Set([current, ...seasons])).sort().reverse();
}

function copyTeacherClassesToSeason(userId, fromSeason, toSeason) {
  const source = listTeacherClassesForSeason(userId, fromSeason);
  ...
  const copy = db.transaction(() => {
    source.forEach((teacherClass, index) => {
      insert.run(userId, teacherClass.name, teacherClass.grade, teacherClass.gender, toSeason, teacherClass.schoolId, index, teacherClass.studentCount, JSON.stringify(teacherClass.roster || []), timestamp, timestamp);
    });
  });
  copy();
  return listTeacherClassesForSeason(userId, toSeason);
}

function listTeacherClasses(userId, options = {}) {
  const selectedSeason = normalizeSeason(options.season);
  let classes = listTeacherClassesForSeason(userId, selectedSeason);
  const canAutoCopy = selectedSeason === seasonForDate() && !classes.length;
  if (canAutoCopy) {
    classes = copyTeacherClassesToSeason(userId, previousSeason(selectedSeason), selectedSeason);
  }
  const seasons = listTeacherSeasons(userId);
  return {
    classes,
    season: selectedSeason,
    activeSeason: seasonForDate(),
    seasons: seasons.includes(selectedSeason) ? seasons : [selectedSeason, ...seasons],
    locked: isSeasonLocked(selectedSeason),
  };
}
...
function getTeacherClass(userId, classId) {
  ...
  return serializeTeacherClass(row);
}
...
function createTeacherClass(userId, payload) {
  ...
  const count = db.prepare('SELECT COUNT(*) AS count FROM teacher_classes WHERE user_id = ? AND season = ?').get(userId, season).count;
  ...
  const timestamp = nowIso();
  const maxOrder = db.prepare('SELECT COALESCE(MAX(order_index), -1) AS max_order FROM teacher_classes WHERE user_id = ? AND season = ?').get(userId, season).max_order;
...
function updateTeacherClass(userId, classId, payload) {
  ...
  if (isSeasonLocked(existing.season)) {
    throw new Error('SEASON_LOCKED');
  }
...
function reorderTeacherClasses(userId, orderedIds) {
  const classes = orderedIds.map((classId) => getTeacherClass(userId, classId)).filter(Boolean);
  if (classes.some((teacherClass) => isSeasonLocked(teacherClass.season))) {
    throw new Error('SEASON_LOCKED');
  }
  ...
  return listTeacherClasses(userId, { season: classes[0]?.season });
}

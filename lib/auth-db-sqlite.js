...
function seedGlobalAdmin() {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, 'admin', ?, ?, ?, 1, 1, ?, ?)
  `).run(email, bcrypt.hashSync(password, 10), `${firstName} ${lastName}`, firstName, lastName, timestamp, timestamp);
}
...
function enableAdminTwoFactor(userId, recoveryCodes) {
  ...
  const save = db.transaction((codes) => {
    ...
    db.prepare('DELETE FROM admin_2fa_recovery_codes WHERE user_id = ?').run(userId);
    const insert = db.prepare('INSERT INTO admin_2fa_recovery_codes (code_hash, user_id, created_at) VALUES (?, ?, ?)');
...
function exportBackupData() {
  return {
    exportedAt: nowIso(),
    users: db.prepare('SELECT id, email, password_hash, role, full_name, first_name, last_name, phone, city, school_name, must_change_password, totp_secret, totp_enabled, totp_enabled_at, is_active, created_at, updated_at FROM users ORDER BY id').all(),
...
function createSession(userId) {
  ...
  db.prepare(`
    INSERT INTO sessions (token, user_id, csrf_token_hash, expires_at, created_at)
    VALUES (?, ?, ?, ?, ?)
  `).run(token, userId, csrfTokenHash, expiresAt, createdAt);

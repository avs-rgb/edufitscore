...
async function initSchema() {
  ...
  await query('ALTER TABLE users ADD COLUMN IF NOT EXISTS trial_started_at TEXT');
  await query('ALTER TABLE users ADD COLUMN IF NOT EXISTS trial_ends_at TEXT');
  await query("ALTER TABLE users ADD COLUMN IF NOT EXISTS billing_status TEXT NOT NULL DEFAULT 'exempt'");
  await query('ALTER TABLE users ADD COLUMN IF NOT EXISTS billing_exempt INTEGER NOT NULL DEFAULT 1');
  await query('ALTER TABLE users ADD COLUMN IF NOT EXISTS billing_exempt_at TEXT');
  await query('ALTER TABLE users ADD COLUMN IF NOT EXISTS billing_exempt_by_user_id INTEGER');
  await query('ALTER TABLE users ADD COLUMN IF NOT EXISTS paid_until TEXT');
  await query("ALTER TABLE users ADD COLUMN IF NOT EXISTS payment_customer_id TEXT NOT NULL DEFAULT ''");
  await query("ALTER TABLE users ADD COLUMN IF NOT EXISTS payment_session_id TEXT NOT NULL DEFAULT ''");
  await query('ALTER TABLE users ADD COLUMN IF NOT EXISTS billing_updated_at TEXT');
  await query("UPDATE users SET billing_exempt = 1, billing_status = 'exempt', billing_exempt_at = COALESCE(billing_exempt_at, created_at), billing_updated_at = COALESCE(billing_updated_at, updated_at) WHERE role = 'admin'");
  await query("UPDATE users SET billing_exempt = 0, billing_status = 'active', billing_exempt_at = NULL, billing_exempt_by_user_id = NULL, paid_until = COALESCE(paid_until, $1), billing_updated_at = $2 WHERE role != 'admin' AND billing_exempt = 1", [billingAcademicYearEndForDate(), nowIso()]);
  await query("UPDATE users SET billing_exempt = 0, billing_status = 'active', paid_until = COALESCE(paid_until, $1), billing_updated_at = COALESCE(billing_updated_at, updated_at) WHERE role != 'admin' AND billing_updated_at IS NULL", [billingAcademicYearEndForDate()]);
  ...
  await query("ALTER TABLE teacher_classes ADD COLUMN IF NOT EXISTS season TEXT NOT NULL DEFAULT '2025-2026'");
  await query("UPDATE teacher_classes SET season = '2025-2026' WHERE season IS NULL OR season = ''");
  await query('CREATE INDEX IF NOT EXISTS teacher_classes_user_season_idx ON teacher_classes(user_id, season, order_index)');
  ...
  await query("DELETE FROM users WHERE email IN ('teacher@edufitscore.local')");
...
async function serializeUser(row) {
  ...
  return {
    ...
    canEnterScores: row.role === 'admin' || row.role === 'teacher' || Boolean(schoolAdminMembership) || Boolean(approvedTeacherSchool),
    billing,
...
async function createUser(payload) {
  ...
  const result = await readyQuery(`
    ...
    VALUES ($1, $2, 'teacher', $3, $4, $5, $6, $7, $8, 0, NULL, 1, $9, $10, 'trialing', 0, $11, $12, $13)
    RETURNING *
  `, [email, bcrypt.hashSync(payload.password, 10), `${firstName} ${lastName}`.trim(), firstName, lastName, String(payload.phone || '').trim(), String(payload.city || '').trim(), String(payload.schoolName || '').trim(), timestamp, trialEndsAt, timestamp, timestamp, timestamp]);
...
async function getUserById(userId) {
  const result = await readyQuery('SELECT * FROM users WHERE id = $1', [userId]);
  return result.rows[0] ? await serializeUser(result.rows[0]) : null;
}

async function setUserBillingExempt(userId, exempt, adminUserId = null) {
  ...
  const result = await readyQuery(`
    UPDATE users
    SET billing_exempt = $1, billing_status = $2, billing_exempt_at = $3, billing_exempt_by_user_id = $4, billing_updated_at = $5, updated_at = $6
    WHERE id = $7
    RETURNING *
  `, [exempt ? 1 : 0, exempt ? 'exempt' : 'trialing', exempt ? timestamp : null, exempt ? adminUserId : null, timestamp, timestamp, userId]);
  return await serializeUser(result.rows[0]);
}

async function extendUserTrial(userId, days = 30) {
  const existing = await readyQuery('SELECT * FROM users WHERE id = $1', [userId]);
  const row = existing.rows[0];
  if (!row) throw new Error('USER_NOT_FOUND');
  const timestamp = nowIso();
  ...
  const result = await readyQuery(`
    UPDATE users
    SET trial_started_at = COALESCE(trial_started_at, $1), trial_ends_at = $2, billing_status = 'trialing', billing_updated_at = $3, updated_at = $4
    WHERE id = $5
    RETURNING *
  `, [timestamp, trialEndsAt, timestamp, timestamp, userId]);
  return await serializeUser(result.rows[0]);
}

async function extendUserToAcademicYearEnd(userId, value = new Date()) {
  const existing = await readyQuery('SELECT * FROM users WHERE id = $1', [userId]);
  ...
  const result = await readyQuery(`
    UPDATE users
    SET paid_until = $1, billing_status = 'active', billing_updated_at = $2, updated_at = $3
    WHERE id = $4
    RETURNING *
  `, [paidUntil, timestamp, timestamp, userId]);
  return await serializeUser(result.rows[0]);
}

async function cancelUserAcademicYearAccess(userId) {
  const existing = await readyQuery('SELECT * FROM users WHERE id = $1', [userId]);
  const row = existing.rows[0];
  if (!row) throw new Error('USER_NOT_FOUND');
  ...
  const result = await readyQuery(`
    UPDATE users
    SET paid_until = NULL, trial_started_at = NULL, trial_ends_at = NULL, billing_status = 'expired', billing_exempt = 0, billing_updated_at = $1, updated_at = $2
    WHERE id = $3
    RETURNING *
...
async function savePaymentSession(userId, { customerId = '', sessionId = '' } = {}) {
  const timestamp = nowIso();
  const result = await readyQuery(`
    UPDATE users
    SET payment_customer_id = COALESCE(NULLIF($1, ''), payment_customer_id), payment_session_id = $2, billing_updated_at = $3, updated_at = $4
    WHERE id = $5
    RETURNING *
  `, [String(customerId || ''), String(sessionId || ''), timestamp, timestamp, userId]);
  return result.rows[0] ? await serializeUser(result.rows[0]) : null;
}

async function markUserPaidForAcademicYear(userId, { customerId = '', sessionId = '', paidAt = new Date() } = {}) {
  const existing = await readyQuery('SELECT * FROM users WHERE id = $1', [userId]);
  if (!existing.rows[0]) throw new Error('USER_NOT_FOUND');
  const timestamp = nowIso();
  ...
  const result = await readyQuery(`
    UPDATE users
    SET paid_until = $1, billing_status = 'active', billing_exempt = 0, payment_customer_id = COALESCE(NULLIF($2, ''), payment_customer_id), payment_session_id = COALESCE(NULLIF($3, ''), payment_session_id), billing_updated_at = $4, updated_at = $5
    WHERE id = $6
    RETURNING *
  `, [paidUntil, String(customerId || ''), String(sessionId || ''), timestamp, timestamp, userId]);
  return await serializeUser(result.rows[0]);
}
...
async function hasRecentPasswordResetToken(email, sinceIso) {
  const row = await findUserByEmail(email);
  if (!row) return false;
  const result = await readyQuery(`
    SELECT 1
    ...
    LIMIT 1
  `, [row.id, sinceIso || new Date(0).toISOString()]);
  return Boolean(result.rows[0]);
}
...
async function logAdminAction(adminUserId, targetUserId, action, details = {}) {
  ...
  if (retentionDays > 0) {
    await readyQuery('DELETE FROM admin_audit_log WHERE created_at < $1', [new Date(Date.now() - (retentionDays * 24 * 60 * 60 * 1000)).toISOString()]);
  }
...
async function listAdminAuditLogFiltered(options = {}) {
  ...
  if (actions.length) {
    where.push(`admin_audit_log.action = ANY(${addParam(actions)})`);
  }
  if (options.from) {
    where.push(`admin_audit_log.created_at >= ${addParam(new Date(`${options.from}T00:00:00`).toISOString())}`);
  }
  if (options.to) {
    where.push(`admin_audit_log.created_at <= ${addParam(new Date(`${options.to}T23:59:59`).toISOString())}`);
  }
  if (search) {
    const term = addParam(`%${search}%`);
    where.push(`(LOWER(admin_audit_log.action) LIKE ${term} OR LOWER(admin.email) LIKE ${term} OR LOWER(admin.full_name) LIKE ${term} OR LOWER(target.email) LIKE ${term} OR LOWER(target.full_name) LIKE ${term} OR LOWER(admin_audit_log.details_json::text) LIKE ${term})`);
  }

  const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
  const countResult = await readyQuery(`
    SELECT COUNT(*) AS count
    FROM admin_audit_log
    LEFT JOIN users admin ON admin.id = admin_audit_log.admin_user_id
    LEFT JOIN users target ON target.id = admin_audit_log.target_user_id
  ...
  const rowsResult = await readyQuery(`
    SELECT admin_audit_log.*, admin.email AS admin_email, admin.full_name AS admin_name, target.email AS target_email, target.full_name AS target_name
    FROM admin_audit_log
    LEFT JOIN users admin ON admin.id = admin_audit_log.admin_user_id
    LEFT JOIN users target ON target.id = admin_audit_log.target_user_id
...
async function countRecentAdminAuditActions(action, detailsMatch = {}, sinceIso = '') {
  const result = await readyQuery('SELECT details_json FROM admin_audit_log WHERE action = $1 AND created_at >= $2', [action, sinceIso || new Date(0).toISOString()]);
...
async function incrementRateLimit(key, windowMs) {
  ...
  await readyQuery('DELETE FROM rate_limits WHERE reset_at <= $1', [now]);
  const result = await readyQuery(`
  ...
  `, [cleanKey, resetAt, now]);
  return { count: Number(result.rows[0].count || 0), resetAt: result.rows[0].reset_at };
}
...
async function restoreBackupData(backup, adminUserId = null) {
  ...
  await readyTransaction(async (client) => {
    ...
    for (const user of backup.users) {
      ...
      `, [user.id, user.email, user.password_hash || bcrypt.hashSync(crypto.randomBytes(12).toString('hex'), 10), user.role, fullName, firstName, lastName, user.phone || '', user.city || '', user.school_name || user.schoolName || '', user.must_change_password || user.mustChangePassword ? 1 : 0, user.totp_secret || user.totpSecret || '', user.totp_enabled || user.totpEnabled ? 1 : 0, user.totp_enabled_at || user.totpEnabledAt || null, user.email_verified === undefined ? (user.emailVerified === false ? 0 : 1) : (user.email_verified ? 1 : 0), user.email_verified_at || user.emailVerifiedAt || null, user.is_active === undefined ? (user.isActive === false ? 0 : 1) : (user.is_active ? 1 : 0), user.trial_started_at || user.trialStartedAt || null, user.trial_ends_at || user.trialEndsAt || null, user.billing_status || user.billingStatus || 'exempt', user.billing_exempt === undefined ? (user.billingExempt === false ? 0 : 1) : (user.billing_exempt ? 1 : 0), user.billing_exempt_at || user.billingExemptAt || (user.billing_exempt || user.billingExempt ? (user.created_at || user.createdAt || nowIso()) : null), user.billing_exempt_by_user_id || user.billingExemptByUserId || null, user.paid_until || user.paidUntil || null, user.payment_customer_id || user.paymentCustomerId || user.stripe_customer_id || user.stripeCustomerId || '', user.payment_session_id || user.paymentSessionId || user.stripe_checkout_session_id || user.stripeCheckoutSessionId || '', user.billing_updated_at || user.billingUpdatedAt || user.updated_at || user.updatedAt || nowIso(), user.created_at || user.createdAt || nowIso(), user.updated_at || user.updatedAt || nowIso()]);
    }
...
async function deleteUserSessionByPublicId(userId, sessionId, currentToken = '') {
  const result = await readyQuery('SELECT token FROM sessions WHERE user_id = $1', [userId]);
  const row = result.rows.find((session) => publicSessionId(session.token) === sessionId);
  if (!row || row.token === currentToken) {
    return false;
  }
  const deleted = await readyQuery('DELETE FROM sessions WHERE user_id = $1 AND token = $2', [userId, row.token]);
...
async function listTeacherClassesForSeason(userId, season) {
  const result = await readyQuery('SELECT * FROM teacher_classes WHERE user_id = $1 AND season = $2 ORDER BY order_index ASC, updated_at DESC, id DESC', [userId, season]);
  return result.rows.map(serializeTeacherClass);
}

async function listTeacherSeasons(userId) {
  const result = await readyQuery('SELECT DISTINCT season FROM teacher_classes WHERE user_id = $1 ORDER BY season DESC', [userId]);
  const current = seasonForDate();
  return Array.from(new Set([current, ...result.rows.map((row) => row.season || initialSeason)])).sort().reverse();
}

async function copyTeacherClassesToSeason(userId, fromSeason, toSeason) {
  const source = await listTeacherClassesForSeason(userId, fromSeason);
  if (!source.length) return [];
  const timestamp = nowIso();
  await readyTransaction(async (client) => {
    for (const [index, teacherClass] of source.entries()) {
      await client.query(`
      ...
      `, [userId, teacherClass.name, teacherClass.grade, teacherClass.gender, toSeason, teacherClass.schoolId, index, teacherClass.studentCount, JSON.stringify(teacherClass.roster || []), timestamp, timestamp]);
    }
  });
  return listTeacherClassesForSeason(userId, toSeason);
}

async function listTeacherClasses(userId, options = {}) {
  const selectedSeason = normalizeSeason(options.season);
  let classes = await listTeacherClassesForSeason(userId, selectedSeason);
  if (selectedSeason === seasonForDate() && !classes.length) {
    classes = await copyTeacherClassesToSeason(userId, previousSeason(selectedSeason), selectedSeason);
  }
  const seasons = await listTeacherSeasons(userId);
  return {
    classes,
...
async function createTeacherClass(userId, payload) {
  ...
  const count = await readyQuery('SELECT COUNT(*) AS count FROM teacher_classes WHERE user_id = $1 AND season = $2', [userId, season]);
  if (Number(count.rows[0].count) >= 18) {
    throw new Error('CLASS_LIMIT_REACHED');
  }
  const timestamp = nowIso();
  const maxOrder = await readyQuery('SELECT COALESCE(MAX(order_index), -1) AS max_order FROM teacher_classes WHERE user_id = $1 AND season = $2', [userId, season]);
  ...
  const result = await readyQuery(`
    ...
    RETURNING *
  `, [userId, payload.name, grade, payload.gender, season, schoolId, Number(maxOrder.rows[0].max_order) + 1, studentCount, JSON.stringify(roster), JSON.stringify(values), timestamp, timestamp]);
...
async function reorderTeacherClasses(userId, orderedIds) {
  const classes = await Promise.all(orderedIds.map((classId) => getTeacherClass(userId, classId)));
  if (classes.filter(Boolean).some((teacherClass) => isSeasonLocked(teacherClass.season))) {
    throw new Error('SEASON_LOCKED');
  }
  await readyTransaction(async (client) => {
  ...
  return listTeacherClasses(userId, { season: classes.find(Boolean)?.season });
}

...
function seedUser({ email, password, role, firstName, lastName }) {
  ...
  db.prepare(`
    INSERT INTO users (email, password_hash, role, full_name, first_name, last_name, is_active, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)
  `).run(email, passwordHash, role, `${firstName} ${lastName}`.trim(), firstName, lastName, timestamp, timestamp);
}

seedUser({
  email: 'teacher@edufitscore.local',
  password: 'Teacher123!',
  role: 'teacher',
  firstName: 'Teacher',
  lastName: 'Demo',
